            ...
            items.map((item) => (
              <div key={item.productId} className="flex gap-4 items-center">
                ...
                <Link
                  ...
                  onClick={() => setIsCartOpen(false)}
                  className="relative w-24 h-24 bg-[#ececec] rounded-2xl flex-shrink-0 flex items-center justify-center p-2"
                >
                  <Image
                    src={item.image}
                    fill
                ...
                <div className="flex-1">
                  <Link href={`/product/${item.slug}`} onClick={() => setIsCartOpen(false)}>
                    <h3 className="font-heading text-lg text-[#94B447] hover:underline">{item.name}</h3>
                  </Link>
                  <p className="text-sm font-semibold text-[#94B447] mt-1">{formatTzs(item.price)}</p>
                  <button
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="w-8 h-8 rounded-lg border border-gray-200 text-[#18202D] flex items-center justify-center hover:border-[#18202D] disabled:opacity-30 disabled:cursor-not-allowed"
                    aria-label="Decrease quantity"
                  >
                    −
                  </button>
                  <span className="w-6 text-center text-[15px] font-semibold text-[#18202D]">
                    {item.quantity}
                  </span>
                  <button
                    onClick={() => updateQuantity(item.productId, item.quantity + 1)}
                    className="w-8 h-8 rounded-lg border border-gray-200 text-[#18202D] flex items-center justify-center hover:border-[#18202D]"
