...
export default function StorefrontChrome({
  ...
  // Contact info: use live settings when present, otherwise fall back to the
  // known static values (settings may not have every field populated yet).
  const contactDsm = settings?.contactDsm || 'Grants Care Building';
  const contactMbeya = settings?.contactMbeya || 'Mwanjelwa Tunduma Road';
  const phoneDsm = settings?.phoneDsm || '0743 483 769';
  const phoneMbeya = settings?.phoneMbeya || '0769 017 608';
  const emailInfo = settings?.emailInfo || 'info@globsby.co.tz';
  const emailMarketing = settings?.emailMarketing || 'marketing@globsby.co.tz';
  const instagramUrl = settings?.instagramUrl || 'https://www.instagram.com/globsby_stationers';
  const facebookUrl = settings?.facebookUrl || '#';
  const whatsappUrl = settings?.whatsappUrl || '#';
  const twitterUrl = settings?.twitterUrl || '#';
  const announcementTagline = settings?.announcementText || 'Globsby Stationary - For Top Quality Supplies';

  const announcementItems = [
    announcementTagline,
    `Dar es Salaam office - ${contactDsm} (${phoneDsm})`,
    `Mbeya office - ${contactMbeya} (${phoneMbeya})`,
    'Order from us and pay in this website!',
  ];
             ...
             <div className={`flex items-center transition-all duration-300 overflow-hidden ${isSearchOpen ? 'w-[200px] sm:w-[300px] opacity-100' : 'w-0 opacity-0'}`}>
               <form
                 onSubmit={(e) => {
                   e.preventDefault();
                   if (searchQuery.trim()) {
                     router.push(`/shop/all-supplies?q=${encodeURIComponent(searchQuery.trim())}`);
                   }
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[100] bg-white flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-300">
          ...
          <nav className="flex flex-col items-center gap-8 text-3xl font-heading font-extrabold text-[#18202D]">
            ...
            <Link href="/tracking" onClick={() => setIsMenuOpen(false)} className="hover:text-[#94B447] transition-colors flex items-center gap-4">
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between gap-12 lg:gap-8 pb-16">
          ...
          <div className="flex flex-wrap md:flex-nowrap gap-12 lg:gap-20">
             ...
             <div className="flex flex-col gap-3 max-w-[220px]">
               ...
               <p className="text-sm text-[#18202D]">{contactDsm}</p>
               <p className="text-sm text-[#18202D]">📞 {phoneDsm}</p>
             </div>

             {/* Mbeya Office */}
             <div className="flex flex-col gap-3 max-w-[220px]">
               <h4 className="text-lg font-medium text-white mb-1">Mbeya Office</h4>
               <p className="text-sm text-[#18202D]">{contactMbeya}</p>
               <p className="text-sm text-[#18202D]">📞 {phoneMbeya}</p>
             </div>
             ...
             <div className="flex flex-col gap-6 max-w-[220px]">
               <div>
                  ...
                  <p className="text-sm text-[#18202D]">{emailInfo}</p>
                  <p className="text-sm text-[#18202D]">{emailMarketing}</p>
               </div>
               <div className="flex gap-4">
                  {/* Instagram */}
                  <a href={instagramUrl} target="_blank" rel="noopener noreferrer" className="w-9 h-9 border border-gray-600 rounded-full flex items-center justify-center text-white hover:bg-white hover:text-black transition-colors" title="Instagram">
                     <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
                  </a>
                  {/* Facebook */}
                  <a href={facebookUrl} className="w-9 h-9 border border-gray-600 rounded-full flex items-center justify-center text-white hover:bg-white hover:text-black transition-colors" title="Facebook">
                  ...
                  {/* WhatsApp */}
                  <a href={whatsappUrl} className="w-9 h-9 border border-gray-600 rounded-full flex items-center justify-center text-white hover:bg-white hover:text-black transition-colors" title="WhatsApp">
                     <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
                  </a>
                  {/* Twitter/X */}
                  <a href={twitterUrl} className="w-9 h-9 border border-gray-600 rounded-full flex items-center justify-center text-white hover:bg-white hover:text-black transition-colors" title="X">
