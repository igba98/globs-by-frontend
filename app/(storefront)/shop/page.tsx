    ...
    <div className="w-full flex flex-col items-center space-y-12 pb-20 pt-4">
      ...
      <section className="w-full">
         <div className="flex justify-between items-center mb-8 px-4">
           <span className="text-sm font-bold text-[#18202D]">{filteredProducts.length} Supplies Available</span>
           <select className="bg-[#f8f9fa] rounded-xl px-4 py-2 text-sm font-bold text-[#94B447] outline-none border border-gray-100 focus:border-[#94B447]">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredProducts.map((pkg) => (
               <Link href={`/product/${pkg.id}`} key={pkg.id} className="bg-[#e9ecef] rounded-[2rem] aspect-[4/5] relative flex flex-col items-center group overflow-hidden drop-shadow-sm hover:-translate-y-1 transition-transform border border-[#18202D]/5 p-6">
                 ...
                 <div className="absolute top-0 left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur-sm px-6 py-2 rounded-b-2xl shadow-sm text-[11px] font-bold text-[#94B447] z-10 whitespace-nowrap">
                   {pkg.name}
                 </div>
                 ...
                 <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-[85%] bg-white/95 backdrop-blur-md rounded-2xl px-6 py-3 shadow-lg opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 text-center">
                    <div className="text-[10px] uppercase tracking-wider font-bold text-[#18202D] mb-0.5">{pkg.category}</div>
                    <div className="text-[15px] font-extrabold text-[#94B447]">{pkg.price}</div>
                 </div>
               </Link>
            ))}
         </div>
         ...
         {filteredProducts.length > 0 && (
           <div className="w-full flex justify-center mt-12">
              <button className="px-10 py-4 bg-[#f8f9fa] rounded-full text-[#94B447] font-bold text-sm hover:bg-[#18202D] hover:text-white border border-gray-200 hover:border-[#18202D] transition-colors shadow-sm">
